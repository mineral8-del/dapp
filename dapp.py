...
if not is_market_open(now_kst) and not force_open:
    ...
    if not df_universe.empty:
        ...
        # 💡 API 과부하 방지를 위해 1차적으로 상위 15개만 추출하여 AI 검사
        df_top15 = df_universe.head(15).copy()
        
        feature_list = []
        valid_indices = []
        
        # 각 종목별로 실시간 차트를 그려서 지표를 계산합니다
        for idx, row in df_top15.iterrows():
            ...
            time.sleep(0.06) # KIS API 속도 제한 방어
            
        if feature_list:
            # AI가 예측할 특징표(DataFrame) 만들기 (학습할 때와 동일한 컬럼 순서)
            X_live = pd.DataFrame(feature_list)
            X_live = X_live[['시가', '고가', '저가', '종가', '거래량', 'MA_5', 'MA_20', '이격도_20', 'RSI_14', 'Vol_MA_5', '거래량_비율', '몸통_크기', '고가_저가_폭']]
            
            # 🤖 드디어 AI 모델 라이브 추론! (10분 뒤 예상 수익률 예측)
            predictions = ai_model.predict(X_live)
            
            # 예측값을 원래 DataFrame에 병합
            valid_df = df_top15.loc[valid_indices].copy()
            valid_df['AI_예측수익률'] = predictions
            
            # AI 예측 수익률을 10점 만점 스케일로 환산 (가장 예측값이 높은 놈이 10점)
